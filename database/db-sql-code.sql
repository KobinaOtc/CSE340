...
(
  ...
  inv_id integer NOT NULL, -- The vehicle being inquired about
  account_from_id integer NOT NULL, -- The client sending the message
  CONSTRAINT message_pk PRIMARY KEY (message_id)
);

-- Create Foreign Key relationship to the `inventory` table
...
-- Create Foreign Key relationship to the `account` table
ALTER TABLE IF EXISTS public.message
  ADD CONSTRAINT fk_account FOREIGN KEY (account_from_id)
  REFERENCES public.account (account_id) MATCH SIMPLE
  ON UPDATE CASCADE
  ON DELETE CASCADE;

-- Create an index to improve lookup performance for a specific vehicle's messages
CREATE INDEX IF NOT EXISTS idx_message_inv_id ON public.message (inv_id);

-- Create an index to improve lookup performance for a specific account's messages (their sent folder)
CREATE INDEX IF NOT EXISTS idx_message_account_from_id ON public.message (account_from_id);